...
{
    private readonly IDbContextFactory<AppDbContext> _dbFactory; private string? _logoPath;
    ...
    {
        await using var db = await _dbFactory.CreateDbContextAsync(); var branch = await db.Branches.Where(x => x.IsActive).OrderBy(x => x.Code).FirstOrDefaultAsync(); BranchText.Text = branch is null ? "Not configured" : $"{branch.Code} — {branch.Name}"; var settings = branch is null ? await db.StoreSettings.FirstOrDefaultAsync() : await db.StoreSettings.FirstOrDefaultAsync(x => x.BranchId == branch.Id); if (settings is null) return; StoreNameBox.Text = settings.StoreName; ArabicNameBox.Text = settings.StoreNameArabic ?? string.Empty; CurrencyBox.SelectedValue = settings.CurrencyCode; CountryBox.Text = settings.Country; CityBox.Text = settings.City; PhoneBox.Text = settings.Phone; MobileBox.Text = settings.Mobile; EmailBox.Text = settings.Email; WebsiteBox.Text = settings.Website; AddressBox.Text = settings.Address; _logoPath = settings.LogoPath; LoadLogo();
    }
    ...
    {
        try { if (string.IsNullOrWhiteSpace(StoreNameBox.Text)) throw new InvalidOperationException("Store name is required."); if (CurrencyBox.SelectedValue is not string currency) throw new InvalidOperationException("Select a currency."); await using var db = await _dbFactory.CreateDbContextAsync(); var branch = await db.Branches.Where(x => x.IsActive).OrderBy(x => x.Code).FirstOrDefaultAsync() ?? throw new InvalidOperationException("No active branch is configured."); var settings = await db.StoreSettings.FirstOrDefaultAsync(x => x.BranchId == branch.Id); if (settings is null) { settings = new StoreSettings(StoreNameBox.Text, currency); settings.AssignBranch(branch.Id); db.StoreSettings.Add(settings); } settings.ConfigureIdentity(StoreNameBox.Text, currency, ArabicNameBox.Text); settings.ConfigureLogo(_logoPath); settings.ConfigureContact(AddressBox.Text, CountryBox.Text, CityBox.Text, PhoneBox.Text, MobileBox.Text, EmailBox.Text, WebsiteBox.Text); await db.SaveChangesAsync(); StatusText.Text = "Store settings saved successfully."; } catch (Exception ex) { StatusText.Text = ex.InnerException?.Message ?? ex.Message; }
    }
