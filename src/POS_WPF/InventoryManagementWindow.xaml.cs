...
public partial class InventoryManagementWindow : Window
{
    ...
    {
        if (WarehouseBox.SelectedValue is not Guid warehouseId) return; await using var db = await _dbFactory.CreateDbContextAsync(); var products = await db.Products.AsNoTracking().Where(x => x.IsActive).Include(x => x.Units).OrderBy(x => x.Name).ToListAsync(); var categories = await db.Categories.AsNoTracking().ToDictionaryAsync(x => x.Id, x => x.Name); var rows = new List<StockRow>(); foreach (var p in products) { var baseUnit = p.Units.SingleOrDefault(x => x.Id == p.BaseUnitId) ?? p.Units.SingleOrDefault(x => x.IsBaseUnit); var stock = await _balances.GetBaseBalanceAsync(p.Id, warehouseId); rows.Add(new StockRow(p.Id, baseUnit?.Id ?? Guid.Empty, p.Sku, p.Name, p.CategoryId is Guid categoryId && categories.TryGetValue(categoryId, out var category) ? category : "—", baseUnit?.Abbreviation ?? "PCS", stock)); } _rows = rows; StockGrid.ItemsSource = _rows;
    }
