...
{
    private readonly IDbContextFactory<AppDbContext> _dbFactory; private readonly UnitConversionService _conversion; private readonly InventoryBalanceService _inventory; private readonly PermissionService _permissions; private readonly SessionContext _session; private Guid? _selectedId; private readonly ObservableCollection<ProductUnit> _units = []; private List<ProductListItem> _products = [];
    ...
    private async void Refresh_Click(object sender, RoutedEventArgs e) { await LoadCategoriesAsync(); await LoadWarehousesAsync(); await LoadAsync(); }
    ...
    private async void Product_Selected(object sender, SelectionChangedEventArgs e)
    {
        ...
        try
        {
            _selectedId = item.Id; SkuBox.Text = item.Sku; NameBox.Text = item.Name; ArabicNameBox.Text = item.NameArabic ?? string.Empty; OpeningStockBox.Text = "0"; OpeningStockBox.IsEnabled = false;
            await using var db = await _dbFactory.CreateDbContextAsync(); var product = await db.Products.AsNoTracking().Include(x => x.Units).SingleAsync(x => x.Id == item.Id); CategoryBox.SelectedValue = product.CategoryId;
            _units.Clear(); foreach (var unit in product.Units.OrderByDescending(x => x.IsBaseUnit).ThenBy(x => x.Name)) _units.Add(unit);
            var warehouse = await db.Warehouses.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Code).FirstOrDefaultAsync(); var stock = warehouse is null ? 0m : await _inventory.GetBaseBalanceAsync(product.Id, warehouse.Id); var baseUnit = product.Units.SingleOrDefault(x => x.Id == product.BaseUnitId) ?? product.Units.SingleOrDefault(x => x.IsBaseUnit);
            if (warehouse is not null) WarehouseBox.SelectedValue = warehouse.Id; CurrentStockText.Text = stock.ToString("N3"); StockUnitText.Text = $" {baseUnit?.Abbreviation ?? "PCS"}"; StatusText.Foreground = System.Windows.Media.Brushes.DimGray; StatusText.Text = string.Empty;
        }
    ...
    private async void Save_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                var existing = await db.Products.SingleAsync(x => x.Id == productId); existing.Sku = product.Sku; existing.Name = product.Name; existing.NameArabic = product.NameArabic; existing.CategoryId = categoryId; existing.BaseUnitId = baseUnit.Id; var existingUnits = await db.ProductUnits.Where(x => x.ProductId == productId).ToListAsync(); var keepIds = _units.Select(x => x.Id).ToHashSet(); db.ProductUnits.RemoveRange(existingUnits.Where(x => !keepIds.Contains(x.Id))); foreach (var unit in _units) { unit.Product = null!; db.ProductUnits.Update(unit); }
            }
            db.AuditEntries.Add(new AuditEntry(_session.CurrentUser?.Id, _selectedId is null ? "Product.Created" : "Product.Updated", nameof(Product), productId, null, product.Sku, null, null, "Product Management")); await db.SaveChangesAsync(); await LoadAsync(); StatusText.Foreground = System.Windows.Media.Brushes.DarkGreen; StatusText.Text = "Product saved successfully.";
        }
