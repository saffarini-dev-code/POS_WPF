...
public partial class PosWindow
{
    ...
    private void ApplyInventoryAwareProductTemplate()
    {
        ...
        root.SetValue(FrameworkElement.HeightProperty, 132d);
        root.SetValue(FrameworkElement.MarginProperty, new Thickness(0, 0, 10, 10));
    ...
    private async Task<bool> HasSufficientStockAsync(Guid productId, Guid unitId, decimal requestedQuantity)
    {
        ...
        var stock = await db.InventoryTransactions.AsNoTracking()
            .Where(x => x.ProductId == productId && x.WarehouseId == warehouse.Id)
            .SumAsync(x => (decimal?)x.BaseQuantity) ?? 0m;
        var factor = await db.ProductUnits.AsNoTracking()
            .Where(x => x.Id == unitId && x.ProductId == productId)
            .Select(x => (decimal?)x.ConversionFactorToBase)
            .SingleOrDefaultAsync() ?? 1m;
    ...
    private async Task<(decimal Stock, decimal Reorder, string Barcode, string Unit)> GetProductCardInfoAsync(Guid productId)
    {
        ...
        var warehouse = await db.Warehouses.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Code).FirstOrDefaultAsync();
        var stock = warehouse is null ? 0m : await db.InventoryTransactions.AsNoTracking()
            .Where(x => x.ProductId == productId && x.WarehouseId == warehouse.Id)
            .SumAsync(x => (decimal?)x.BaseQuantity) ?? 0m;
    ...
    public sealed class ProductCardStockInfo : StackPanel
    {
        ...
        {
            if (d is ProductCardStockInfo control && control.IsLoaded) _ = control.RefreshAsync();
        }
