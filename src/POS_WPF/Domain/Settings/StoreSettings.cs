...
public sealed class InvoiceFooterLine { private InvoiceFooterLine() { } public InvoiceFooterLine(string text, int sortOrder) { Id = Guid.NewGuid(); Text = text.Trim(); SortOrder = sortOrder; } public Guid Id { get; private set; } public string Text { get; private set; } = string.Empty; public int SortOrder { get; private set; } public bool IsEnabled { get; private set; } = false; }
public sealed class TaxSettings : Entity { public TaxSettings() { } public bool IsEnabled { get; private set; } = false; public decimal Rate { get; private set; } public bool PricesIncludeTax { get; private set; } public bool ShowTaxOnInvoice { get; private set; } = true; public void Configure(bool enabled, decimal rate, bool pricesIncludeTax, bool showTaxOnInvoice) { if (rate < 0 || rate > 100) throw new ArgumentOutOfRangeException(nameof(rate)); IsEnabled = enabled; Rate = rate; PricesIncludeTax = pricesIncludeTax; ShowTaxOnInvoice = showTaxOnInvoice; UpdatedAtUtc = DateTime.UtcNow; } }
