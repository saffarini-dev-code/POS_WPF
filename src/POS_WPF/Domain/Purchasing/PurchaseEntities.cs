...
public sealed class Purchase : Entity
{
    ...
    public void AddLine(Guid productId, Guid unitId, decimal quantity, decimal unitCost, decimal discount, decimal tax)
    {
        if (quantity <= 0 || unitCost < 0 || discount < 0 || tax < 0) throw new ArgumentOutOfRangeException(nameof(quantity));
        _lines.Add(new PurchaseLine(productId, unitId, quantity, unitCost, discount, tax));
        Subtotal = _lines.Sum(x => x.Quantity * x.UnitCost);
...
public sealed class PurchaseLine
{
    ...
    internal PurchaseLine(Guid productId, Guid unitId, decimal quantity, decimal unitCost, decimal discount, decimal tax)
    { Id = Guid.NewGuid(); ProductId = productId; UnitId = unitId; Quantity = quantity; UnitCost = unitCost; Discount = discount; Tax = tax; }
