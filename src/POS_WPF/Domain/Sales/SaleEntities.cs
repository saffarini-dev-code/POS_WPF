...
public sealed class Sale : Entity
{
    ...
    public void AddLine(Guid productId, Guid unitId, string description, decimal quantity, decimal unitPrice, decimal discount, decimal tax)
    {
        if (quantity <= 0) throw new ArgumentOutOfRangeException(nameof(quantity));
        if (unitPrice < 0 || discount < 0 || tax < 0) throw new ArgumentOutOfRangeException(nameof(unitPrice));
        _lines.Add(new SaleLine(productId, unitId, description, quantity, unitPrice, discount, tax));
        Recalculate();
    }
...
{
    private SaleLine() { }
    internal SaleLine(Guid productId, Guid unitId, string description, decimal quantity, decimal unitPrice, decimal discount, decimal tax)
    {
        Id = Guid.NewGuid(); ProductId = productId; UnitId = unitId; Description = description.Trim(); Quantity = quantity; UnitPrice = unitPrice; Discount = discount; Tax = tax;
    }
