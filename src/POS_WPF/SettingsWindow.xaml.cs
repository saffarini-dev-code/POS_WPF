...
public partial class SettingsWindow : Window
{
    ...
    private async void Save_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (settings is null) { settings = new StoreSettings(StoreNameBox.Text, CurrencyBox.Text); settings.AssignBranch(branch.Id); settings.ConfigureIdentity(StoreNameBox.Text, CurrencyBox.Text, ArabicNameBox.Text); db.StoreSettings.Add(settings); } else settings.ConfigureIdentity(StoreNameBox.Text, CurrencyBox.Text, ArabicNameBox.Text);
            ...
            await db.SaveChangesAsync(); _localization.SetLanguage(LanguageBox.SelectedIndex == 1 ? AppLanguage.Arabic : AppLanguage.English); StatusText.Text = "Saved.";
        }
