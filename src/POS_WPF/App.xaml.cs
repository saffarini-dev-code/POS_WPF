...
public partial class App : Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        _host = Host.CreateDefaultBuilder().ConfigureServices((context, services) =>
        {
            ...
            services.AddSingleton<CrashLogger>(); services.AddSingleton<UnitConversionService>(); services.AddSingleton<UnitConversionGraph>(); services.AddSingleton<InventoryService>(); services.AddSingleton<InventoryBalanceService>(); services.AddSingleton<InventoryOperationsService>(); services.AddSingleton<OpeningStockService>(); services.AddSingleton<PricingCalculator>(); services.AddSingleton<LocalizationService>();
    ...
    private static async Task EnsureSchemaAsync(AppDbContext db)
    {
        ...
        async Task<bool> ColumnExists(string table, string column) { command.CommandText = isSqlServer ? $"SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='{table}' AND COLUMN_NAME='{column}'" : $"SELECT COUNT(*) FROM pragma_table_info('{table}') WHERE name='{column}'"; return Convert.ToInt32(await command.ExecuteScalarAsync()) > 0; }
        async Task AddColumn(string table, string column, string sqlServerType, string sqliteType) { if (await ColumnExists(table, column)) return; command.CommandText = isSqlServer ? $"ALTER TABLE [{table}] ADD [{column}] {sqlServerType} NOT NULL DEFAULT 0" : $"ALTER TABLE {table} ADD COLUMN {column} {sqliteType} NOT NULL DEFAULT 0"; await command.ExecuteNonQueryAsync(); }
        await AddColumn("ProductUnits", "WholesalePrice", "decimal(18,2)", "decimal(18,2)");
        await AddColumn("ProductUnits", "WholesaleWholesalePrice", "decimal(18,2)", "decimal(18,2)");
        await AddColumn("Promotions", "ProductId", "uniqueidentifier NULL", "TEXT");
        await AddColumn("Promotions", "UnitId", "uniqueidentifier NULL", "TEXT");
        await AddColumn("Promotions", "MaxQuantity", "decimal(18,6) NULL", "decimal(18,6)");
        command.CommandText = isSqlServer ? "IF OBJECT_ID('HeldSales','U') IS NULL CREATE TABLE HeldSales (Id uniqueidentifier NOT NULL PRIMARY KEY, CreatedAtUtc datetime2 NOT NULL, UpdatedAtUtc datetime2 NULL, Reference nvarchar(80) NOT NULL, CashierId uniqueidentifier NOT NULL, Payload nvarchar(max) NOT NULL, IsReleased bit NOT NULL DEFAULT 0)" : "CREATE TABLE IF NOT EXISTS HeldSales (Id TEXT NOT NULL PRIMARY KEY, CreatedAtUtc TEXT NOT NULL, UpdatedAtUtc TEXT NULL, Reference TEXT NOT NULL, CashierId TEXT NOT NULL, Payload TEXT NOT NULL, IsReleased INTEGER NOT NULL DEFAULT 0)"; await command.ExecuteNonQueryAsync();
    }
