...
public partial class AccountsWindow : Window
{
    ...
    { await using var db = await _dbFactory.CreateDbContextAsync(); if (PartyTypeBox.SelectedIndex == 1) PartyGrid.ItemsSource = await db.Suppliers.AsNoTracking().OrderBy(x => x.Name).Select(x => new { x.Id, x.Code, x.Name, x.Phone, x.CurrentBalance }).ToListAsync(); else PartyGrid.ItemsSource = await db.Customers.AsNoTracking().OrderBy(x => x.Name).Select(x => new { x.Id, x.Code, x.Name, x.Phone, x.CreditLimit, x.CurrentBalance }).ToListAsync(); }
    private async void Refresh_Click(object sender, RoutedEventArgs e) => await LoadAsync();
    private async void Party_Selected(object sender, SelectionChangedEventArgs e)
    { if (PartyGrid.SelectedItem is null) return; var property = PartyGrid.SelectedItem.GetType().GetProperty("Id"); _selectedId = property?.GetValue(PartyGrid.SelectedItem) as Guid?; if (_selectedId.HasValue) await LoadStatementAsync(); }
    private async Task LoadStatementAsync()
    { if (!_selectedId.HasValue) return; var service = new AccountStatementService(_dbFactory); var type = PartyTypeBox.SelectedIndex == 1 ? AccountPartyType.Supplier : AccountPartyType.Customer; StatementGrid.ItemsSource = await service.GetAsync(type, _selectedId.Value, DateTime.UtcNow.AddYears(-1), DateTime.UtcNow); }
