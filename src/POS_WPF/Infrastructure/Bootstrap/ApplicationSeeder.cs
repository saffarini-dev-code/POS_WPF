...
public sealed class ApplicationSeeder(AppDbContext db, IPasswordHasher passwordHasher)
{
    public async Task SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        var superRole = await db.Roles.SingleAsync(x => x.Name == PermissionCatalog.SuperAdministrator, cancellationToken); var admin = await db.Users.SingleOrDefaultAsync(x => x.Username == "admin", cancellationToken);
        ...
        {
            var branch = new Branch("MAIN", "Main Branch"); db.Branches.Add(branch); await db.SaveChangesAsync(cancellationToken); db.Warehouses.Add(new Warehouse(branch.Id, "MAIN", "Main Warehouse")); db.Terminals.Add(new Terminal(branch.Id, "POS-01", "POS Terminal 01")); db.CashRegisters.Add(new CashRegister(branch.Id, "REG-01", "Cash Register 01")); db.StoreSettings.Add(new StoreSettings("Retail POS", "JOD") { }); db.InvoiceSettings.Add(new InvoiceSettings()); db.TaxSettings.Add(new TaxSettings());
        }
